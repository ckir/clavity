...
- **Related but distinct backlog entries** - `agy-look-tail-truncation` (a bounded read-back truncating the
  ...
  that fails every call until the conversation is abandoned. [!] **This line said "all currently open" until
...
[!] **Ships WITH the hint fix.** Raising the cap alone leaves `ChannelDown`'s unconditional message
...
[!] **The status value is `resource_exhausted`, NOT `payload_too_large`.** The first implementation used
...
[!] **The live half was NOT run, and this is stated rather than glossed.** The plan's optional Step 3c would
