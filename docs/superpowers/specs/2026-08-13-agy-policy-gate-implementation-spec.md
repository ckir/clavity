...
| N3 | **Evaluate the skip token FIRST** - a valid `AGY-SKIP: ROLES <reason>` ends the ROLES stage (N25) - **but extract the seam anyway so the log names it.** Requires a readable payload: with no `jq` on the MCP transport the token cannot be found, so the skip is not honoured and the stage ends as a `no-jq` fail-open. | 4, 7, N25 |
| N4 | A hook honours **only** a skip naming its own rule; a bare `AGY-SKIP:` never skips. | 7 |
| N5 | Infrastructure failure **never CAUSES a block, and never OVERRIDES a violation that was detected**. It can and does prevent DETECTION - that is what fail-open means, and round 16 falsified the stronger wording: with no `jq`, extraction fails and a block that should have happened does not. Every error path ends the stage per N25. | 4 |
...
| N25 | **READ EVERY `exit 0` IN THIS DOCUMENT AS "END THE ROLES STAGE", NOT "END THE SCRIPT".** In the standalone form they are the same thing; **in the merged hook they are not, and only `exit 2` ever truly exits.** Rounds 14, 15 and 16 each found one requirement colliding with this - N7, the `mkdir` guard, and the skip short-circuit - **so it is stated once here instead of being patched a fourth time.** | 3b |
...
| N19 | **Fixed order: resolve root -> EXTRACT candidate seam paths -> (the invocation was identified as a consult AND either a seam or a skip was found, OR the payload could not be READ at all?) create `.clavity/` and assert the shield - **the last clause exists only so `no-jq` has somewhere to land, and round 18 was right that dropping it to 'any identified consult' would contradict N7's rule that naming no seam writes nothing** -> evaluate skip -> evaluate seams -> log -> exit.** **Within a repository, and when the payload is readable, a valid skip creates the directory and is logged**, even when it names no seam. (Outside one, N7 has already exited - round 15 caught this stated as an unqualified 'always'.) - round 14 caught that a skip with no seam would have silently lost the one record the hatch's counterweight depends on.** Extraction comes first because the create-or-not branch depends on its result; round 11 caught the earlier ordering asking a question before the step that answers it.** The skip short-circuits the DECISION, never the setup that makes logging possible. | 4, 8a |
| N20 | `outside-repo` means **the token resolved under SOME `/.clavity/seams/` but not THIS repository's** - a seam belonging to another checkout. It does NOT cover a path that resolves nowhere near a seams directory (`../../ghost.md`): that names no seam at all and is out of jurisdiction. **Round 18 called the code unreachable; it is reachable but narrower than its name suggests**, which is worth saying because the name invites the broader reading. **A token counts as NAMING A SEAM when its path RESOLVES (lexically - `realpath -m`, no existence required) under some `/.clavity/seams/`.** **Existence is checked AFTERWARDS and is a separate outcome.** Round 17: requiring existence up front made `seam-missing` mathematically unreachable - a misspelled or hallucinated seam would fail the existence test, be classified as naming no seam, and vanish, **which is precisely the case that reason code exists to record** - which is what makes `outside-repo` detectable and in jurisdiction. Without that, round 15 showed containment failure would classify it as no-seam and swallow the diagnostic. | 4a |
| N21 | **EXACTLY one decision line per identified consult, always** - including when every named seam was missing or unreadable, which yields `pass` (nothing was blocked) plus the diagnostics. Round 19 found that path exiting with zero decision lines, **which corrupts the invocation count this row exists to make countable.** One **decision** line per invocation (`block`/`pass`/`skip`), plus zero or more **diagnostic** lines for individually anomalous seams (`seam-missing`, `seam-unreadable`, `outside-repo`). **`no-seam` is NOT a reason code at all** (4a-00): a payload naming no seam is outside this gate's jurisdiction and is never written. The decision set is `block`/`pass`/`skip`. **The two reason-code sets are DISJOINT, which is what keeps them separable** - the reader counts decisions by the first set and anomalies by the second, and needs no extra column. Invocation counts come from decision lines only. | 8 |
...
| N15 | **Validity is checked on the RAW field; sanitisation happens on the way to the log.** A skip is valid or not before any stripping, and stripping never turns an invalid token into a valid one. **Sanitise both agent-authored fields** - the seam path and the skip reason - by stripping control characters **and the TAB delimiter**, then length-capping. Newline-stripping alone leaves horizontal forging open. | 8 |
...
| N26 | **PRE-FILTER candidate tokens before resolving, on a test that is STRICTLY MORE PERMISSIVE THAN THE DECISION: `clavity` and `seams` present, CASE-INSENSITIVE and separator-insensitive (`/` or `\`).** **A literal case-sensitive `.clavity/seams/` was the first version and round 18 killed it** - it silently dropped `.CLAVITY\SEAMS\BAD.MD` before resolution, **reinstating exactly the bypass N22 exists to close.** **A pre-filter may only skip a token that could not possibly resolve into a seams directory; if it can drop a candidate the decision would have caught, it IS the decision, and a worse one.** Round 17 measured the cascade this prevents: without it, N18's no-shape rule forces every whitespace-separated word of a prose payload through the filesystem - **hundreds of resolutions for one commit message, on a hook whose dominant cost is already process startup (3b).** **This is NOT the shape-matching N22 bans:** the substring is a cheap in-process HINT for which tokens are worth resolving; **the decision still comes from resolution and containment, never from the hint.** | 3, 3b |
| N24 | **TOKENISE before resolving, per transport, because N22 forbids shape-matching and N23 requires space support - and round 14 showed those two together left no viable method.** **MCP:** the `\n` join gives one LINE per payload string - **a line is not a token.** Round 19 traced it: a paragraph containing a seam is one line, survives the pre-filter and punctuation stripping, resolves literally, fails existence and **yields `seam-missing` instead of checking the file.** Split each line on whitespace as well. **CLI:** shell-tokenise into arguments, **then split each argument on whitespace as well** - the payload is one quoted argument containing prose, so stopping at the argument boundary would hand a whole sentence to the resolver. **Never resolve a whole line or a whole argument as a path**: it resolves outside the root and yields a silent bypass. **Round 15 caught this rule contradicting itself** - it said the quoted argument is one token AND that a prose line must never be resolved, which left nothing constructible. | 3, 13 |
| N27 | **Strip punctuation from BOTH ENDS of a candidate, and any leading `--flag=` or `key=` prefix, before resolving it.** Round 19: `--file=.clavity/seams/x.md` and `\"(see .clavity/seams/x.md)\"` both survive a trailing-only strip, resolve literally and downgrade a BLOCK to `seam-missing` - **the same one-character class as the trailing full stop, on the other end.** Round 18: a path written naturally in prose - *\"see .clavity/seams/x.md.\"* - tokenises with the sentence's full stop, passes the pre-filter, resolves literally, fails existence, and **downgrades a BLOCK to a non-blocking `seam-missing` diagnostic.** A period is a one-character bypass. | 3, 13 |
| N23 | **A seam path containing a SPACE is extracted on the MCP transport, and is BEST-EFFORT on the CLI one.** On MCP each string is its own token, so a space is no obstacle. **In CLI prose, whitespace tokenisation cannot recover it and shape-matching is banned (N22) - so it is honestly out of reach**, exactly like N2's other transport-scoped guarantee. **The measurement stands and the scope is what changed:** Measured: `[^[:space:]"]*\.clavity/seams/[^[:space:]"]+\.md` silently fails on `.clavity/seams/my seam.md` while extracting the space-free control - **so the obvious regex is a silent bypass anyone can trigger by naming a file normally on Windows.** In the MCP form each string is its own line after the `
...
> **This binds N7 too.** Outside a repository the standalone script exits 0; **in the merged hook it
> falls through instead**, because "write nothing" and "stop the other guard" are different
> instructions and only the first was intended. Round 15 caught N7's unqualified `exit 0` contradicting
> this rule - **the same collision as round 14's, one requirement later, which is why the exception is
> now stated at the rule rather than left to be re-derived.**
>
...
> **A positive violation on ANY named seam BLOCKS, whatever the other SEAMS did** - a valid skip token is not a seam and still short-circuits first (N3). Missing, unreadable
...
```sh
R="$(git rev-parse --show-toplevel 2>/dev/null)" || exit 0   # N25: END THE ROLES STAGE, not the script
[ -n "$R" ] || exit 0                                        # N25: same - the merged hook falls through
mkdir -p "$R/.clavity" || exit 0     # STANDALONE FORM ONLY. In the merged hook this line must END THE
                                     # ROLES STAGE, not the script (N25) - a literal copy of this sample
                                     # kills the existing guard, which round 17 caught surviving the
                                     # prose rule that was supposed to govern it
...
| **a misspelled seam under `.clavity/seams/` yields `seam-missing`, NOT silence** | require existence before classifying it as a seam -> the fixture's typo'd path vanishes and the reason code is never emitted -> row reds. **Pins that the code is reachable at all** |
| **`.CLAVITY\SEAMS\BAD.MD` survives the pre-filter and is decided by resolution** | make the pre-filter case- or separator-sensitive -> the fixture is dropped before resolution and passes -> row reds. **Pins that the pre-filter is more permissive than the decision** |
| **`.clavity/seams/x.md.` with a trailing full stop still BLOCKS** | skip punctuation stripping -> the fixture downgrades to `seam-missing` -> row reds |
| **a prose payload resolves only the tokens containing `.clavity/seams/`** | drop the pre-filter -> count filesystem calls on a 200-word fixture -> row reds. Assert the COUNT, not the wall-clock |
| **`no-jq` is logged on a consult the matcher identified** | gate directory creation on a parsed seam -> the no-jq fixture has nowhere to write and the outcome vanishes -> row reds |
...
| **in the merged classic hook, a SKIP, a PASS and every infrastructure failure all fall through; only a VIOLATION exits 2** | make any of them exit -> assert the existing guard's VCS baseline is still written in each case -> row reds. **Pins N25 across all four paths, not just the one that prompted it** |
...
| **a seam path with a space is extracted on the MCP transport** | tokenise the joined payload on whitespace -> the MCP fixture misses -> row reds. Pins N23's guaranteed half |
| **the CLI transport does NOT claim to extract a space-bearing path** | assert extraction there -> the row fails against a correct implementation, which is worse than a gap: **a test that demands an unreachable guarantee gets 'fixed' by weakening the code** |
...
| **`echo "run clavity ask"` produces NO log line and NO directory** | log or create on it -> row reds. **The row asserts the OUTCOME, not the absence of a subprocess:** the adjacent-word test cannot tell a quoted mention from a real invocation without the shell parser section 3a bans, so a false positive costs one wasted process and that is accepted (4a-00) |
...
### Round 15 - 2 new seats, 6 findings: 5 folded, 1 rejected with a reason

**The seats were built for the N22/N23 failure mode - two individually-correct rules that leave nothing
constructible between them - and they found four more of it.**

| finding | disposition |
|---|---|
| **N24 contradicts itself on the CLI transport** | **folded, and it is the round's point.** It said the quoted argument is ONE token *and* that a prose line must never be resolved - **so for the transport where the payload IS a quoted prose argument, nothing was constructible.** Now: shell-tokenise into arguments, then split each argument on whitespace as well |
| **N23's space guarantee is unreachable on the CLI** | **folded by scoping it, not by inventing a mechanism.** In prose, whitespace tokenisation cannot recover a space-bearing path and shape-matching is banned - **so it is honestly out of reach on that transport**, exactly like N2's guarantee. **The measurement that produced N23 still stands; what changed is the claim** |
| **N7's `exit 0` collides with the merged hook's fall-through rule** | **folded at the rule rather than left to be re-derived.** "Write nothing" and "stop the other guard" are different instructions and only the first was intended. **This is round 14's collision one requirement later** |
| **an out-of-repo seam fails containment, so it is classified no-seam and its diagnostic is swallowed** | **folded via N20:** a token counts as naming a seam when it resolves to an existing file whose path contains `/.clavity/seams/`, whatever repository that is. Without that, the `outside-repo` code was unreachable in exactly the case it names |
| **N19's "a valid skip ALWAYS creates the directory" is false outside a repository** | folded - qualified to "within a repository", where N7 has not already exited. **Another unqualified "always" of mine, one round after the last one** |
| **falling through on infrastructure failure hands control to the existing guard in a degraded environment** | **REJECTED, with the reason.** That guard is fail-open by its own design and already handles a degraded environment; **falling through restores the status quo ante, which is precisely what "do not swallow the other guard" means.** Exiting instead would be the change in behaviour |

### Round 16 - 1 re-seated + 1 new seat, 6 findings: 5 folded, 1 not reproduced

| finding | disposition |
|---|---|
| **the skip's `exit 0` destroys the merged hook's fall-through** | **folded as N25, and the fold is the point.** This is the THIRD requirement to collide with that rule - N7 in round 15, the `mkdir` guard in round 14, the skip now. **Patching a third instance would have guaranteed a fourth**, so every `exit 0` in this document now reads as "end the ROLES stage", and only `exit 2` truly exits |
| **N5's "never prevents a block" is false** | **folded.** With no `jq`, extraction fails and a block that should have happened does not - **an infrastructure failure prevents DETECTION, which is what fail-open means.** The rule is now that it never CAUSES a block and never OVERRIDES a detected one |
| **N19's "a valid skip ALWAYS creates the directory" assumes a readable payload** | folded - it needs `jq` on the MCP transport. **Fourth unqualified "always" of mine in four rounds**, which is why N25 was written as a rule rather than another patch |
| **section 4's "whatever the other seams did" reads as absolute over the skip** | folded: a valid skip is not a seam and still short-circuits first |
| **a test row demanded no subprocess on `echo "run clavity ask"`, precision section 3a bans** | **folded by asserting the OUTCOME instead.** The adjacent-word test cannot tell a quoted mention from a real invocation without the shell parser 3a forbids; the row now pins that nothing is logged or created, and the wasted process is accepted |
| N24 makes N23's space support impossible in CLI prose | **NOT REPRODUCED** - round 15 already scoped N23 to MCP-guaranteed and CLI-best-effort. The two are consistent as they stand |

### Round 17 - 1 new + 1 re-seated seat, 4 findings, all folded. All four caused by rounds 15-16

| finding | disposition |
|---|---|
| **requiring a seam to EXIST made `seam-missing` unreachable** | **folded, and it is the round's sharpest.** Round 15 defined "names a seam" as resolving to an *existing* file - so a misspelled or hallucinated path failed that test, was classified as naming no seam, and vanished. **That is exactly the case `seam-missing` exists to record: the reason code was mathematically dead.** Resolution is now lexical (`realpath -m`); existence is a separate, later question |
| **the no-shape rule forces resolving every word of a prose payload** | **folded as N26**, a cheap in-process substring pre-filter on `.clavity/seams/`. Without it, one commit message means hundreds of filesystem resolutions **on a hook whose dominant cost is already process startup (3b)**. **The pre-filter is a HINT for what is worth resolving, not the decision** - which still comes from resolution and containment, so N22 stands |
| **N25 governs the prose but the literal code sample still said `\|\| exit 0`** | **folded - the same prose-versus-code split that round 9 caught, recurring.** A governing rule reaches every sentence and no code block; the sample now carries the distinction in a comment |
| **`no-jq` had nowhere to be logged** | **folded.** Directory creation was gated on finding a seam or a skip - both of which need the payload parsed - so the one failure that means *"I could not parse the payload"* could never record itself. Creation is now gated on **the invocation being identified as a consult**, which both transports know without parsing |

**Every finding this round was caused by a fix from the previous two.** That is not a reason to stop
folding; it is the reason each fold gets the round after it.

### Round 18 - 2 new seats, 4 findings, all folded. All four caused by round 17

| finding | disposition |
|---|---|
| **N26's case-sensitive pre-filter reinstated the bypass N22 exists to close** | **folded, and it is the round's most serious.** A literal `.clavity/seams/` test silently dropped `.CLAVITY\SEAMS\BAD.MD` **before** resolution - the exact case-and-separator bypass N22 was written to kill, reintroduced one round later by the optimisation meant to make N22 affordable. **The governing principle is now stated: a pre-filter may only skip what could not possibly resolve into a seams directory. If it can drop a candidate the decision would have caught, it IS the decision, and a worse one** |
| **a trailing full stop downgrades a block to a diagnostic** | **folded as N27.** A path written naturally in prose - *"see .clavity/seams/x.md."* - tokenises with the sentence's punctuation, passes the pre-filter, resolves literally and fails existence. **A period is a one-character bypass** |
| **round 17 tagged one `exit 0` in the 8a sample and left two untagged** | **folded - an incomplete fix of the exact thing round 17 fixed.** Both remaining exits now carry the N25 distinction. A sample is what an implementer copies |
| **N19's "any identified consult creates the directory" contradicts N7** | **folded.** Round 17 widened the condition to give `no-jq` somewhere to land and overshot into contradicting the rule that naming no seam writes nothing. The clause is now exactly as wide as that one purpose needs |
| `outside-repo` is unreachable | **PARTLY REFUTED.** It is reachable - a token resolving under ANOTHER checkout's `/.clavity/seams/` - but **narrower than its name suggests**, which is now said, because the name invites the broader reading the seat took |

**Four rounds running, every finding has been a consequence of the previous round's fix - and they are
