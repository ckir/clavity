...
release-lib.Tests.ps1                             5,0s   42 tests   <- FAST, re-measured 2026-08-05; 23->20 on 2026-09-14 (ghidrust full retirement removed the Get-GhidrustChannel Describe); 20->21 on 2026-09-18 (Update-Changelog must emit a pure-ASCII section, so a release cannot re-break the injected-context gate); 21->36 on 2026-09-22 (the four -Resume functions: Get-DanglingReleaseCommits, Test-ResumeState, Get-DropConflictStatus, Invoke-DropReleaseCandidate); 36->37 on 2026-09-22 (AGY-CAPSTONE round 1 fold: merge topology must survive the candidate drop); 37->42 on 2026-09-22 (AGY-TEST-AUDIT round 1 fold: the fail-CLOSED pair for an unresolvable origin/main, the --basic-regexp guard under grep.patternType=extended, the tip-candidate-is-also-root short-circuit, and the catch arm under $PSNativeCommandUseErrorActionPreference). The 5,0s time is the 2026-08-05 figure and was NOT re-measured when those 15 tests landed, so it now understates this row
release.Tests.ps1                                56,5s    6 tests   <- SLOW, ADDED 2026-09-22 with release.ps1 -Resume. The FIRST suite release.ps1 has ever had. SLOW on a measured cadence judgement, not a threshold: at 56,5s it would have been the slowest member of the FAST half (whose largest was 43,2s, and that one carries 112 tests against these 6) and would have added ~75% to a ~75s inner loop. 9,4s per test is structural - each test spawns a pwsh running a COPY of release.ps1 in a throwaway repo with a real bare origin, because release.ps1 derives $RepoRoot from $PSScriptRoot and so always acts on the repo its own copy sits in. MEASURED decomposition: git setup ~3,4s/test, the irreducible pwsh+release.ps1 invocation ~3,9s/test, so amortising setup into BeforeAll would reach ~25-35s, NOT the "<5s" a reviewer predicted. The RESUME LOGIC is not gated here - it lives in release-lib.ps1 and its 42 tests stay FAST; this suite covers only the orchestrator wiring. `just test-scripts` globs the directory, so it runs in the release pre-flight regardless of this half
