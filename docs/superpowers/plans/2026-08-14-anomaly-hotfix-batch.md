...
function Get-IndexBytes {
    ...
    $psi.FileName = 'git'
    $psi.ArgumentList.Add('show'); $psi.ArgumentList.Add(":$Path")
...
function Test-InIndex { param([string]$Path) & git ls-files --error-unmatch -- $Path *> $null; return ($LASTEXITCODE -eq 0) }
```
