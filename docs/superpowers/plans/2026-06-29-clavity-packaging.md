...
- **Task 7.7 — `clavity restart` (agy-only) for clavity-classic.** Add a `restart` subcommand (or `--restart`) that tears down + relaunches ONLY the agy psmux session under the same `--session` name, WITHOUT co-launching a new Claude Code. Today the only relaunch is `clavity start`, which co-launches Claude **and** agy — so there is no way to recover a hung/broken agy (e.g. its MCP servers failing to come up) while KEEPING the current driving Claude/conversation: the operator must `clavity stop` + manually relaunch, and `clavity start` would orphan the session. `restart` should re-run agy's exact launch (its env + MCP config) in the existing session and confirm readiness via a `ping` round-trip. **Surfaced 2026-06-29** when an agy MCP-server hang forced a full `clavity stop` + `tmux kill-server` teardown mid-capstone, and the missing agy-only restart orphaned the driving session.
