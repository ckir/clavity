...
Describe 'check-installer-ascii.ps1' {
    ...
    It 'FAILS with exit 1 and names the offender when the 5.1 domain carries a non-ASCII byte' {
        ...
        try {
            ...
            # The whole report line, anchored at both ends: the path, then the byte count, nothing else. ANSI
            ...
            ($out -replace "`e\[[0-9;]*m", '') | Should -Match '(?m)^  installer\\offender\.ps1 - 1 non-ASCII byte\(s\)\r?$'
        }
