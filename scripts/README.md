...
| `check-ci-filter-coverage.ps1` | Gate: pins a required-entry table for `ci-scripts.yml`'s `paths:` filter — each row carrying WHY it is needed — and asserts every entry appears in BOTH the `push` and `pull_request` blocks (nothing else checks that the two agree). A second half that discovered new trees via the PowerShell AST was deleted by owner decision 2026-08-17: it produced four defects of its own across five capstone rounds against five gaps found once, all five now pinned by the table. Known limit, accepted: a NEW tree asserted against by a NEW suite is not detected here | `pwsh -File scripts/check-ci-filter-coverage.ps1` |
