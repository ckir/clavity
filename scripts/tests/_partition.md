...
- `just test-scripts-fast` — the agent inner-loop gate. **LAST MEASURED 2026-09-09 at `b2f4955`: 494 passed /
  0 failed / 1 skipped in 480,5s** (the section 28 closing sweep, contended - an upper bound). THIRTY-EIGHT rows
  ...
  `rule-runner.Tests.ps1` (ROADMAP section 30, whose `exit` row starts two child pwsh processes); and 16 more in
  `check-agy-discipline-skills.Tests.ps1` (AGY-TEST-AUDIT section 30, rounds 1 and 2), each one in-process linter
  run. This line said "seven" until 2026-09-12 - section 30's fifteen had never been added to it. **Re-measure
  before adding more.** (This line used to read
...
agy-discipline-reaching.Tests.ps1                15,2s   28 tests   <- FAST. The row said 69,1s and it had
...
check-skill-frontmatter.Tests.ps1                   ?   31 tests   <- FAST, added 2026-09-23; REWRITTEN 2026-09-24 for the yq-based lint (34 -> 25 -> 26 -> 27 after capstone R1-R2: the byte-bound and hand-rolled-YAML rows went with the code they tested; -> 31 after the test audit: list/number name, frontmatter not on line 1, no trailing newline, a mikefarah-v5 version case). RUNTIME NOT MEASURED under the timing discipline; one
...
check-peer-reply-citations.Tests.ps1             49,2s   39 tests   <- SLOW, added 2026-09-02 (34 -> 39 on 2026-09-13, §21 citation-checker gaps: empty-quote, NUL-in-file, schema-boundary rows)
  ...
  re-measured here.
  TWO RUNS, RANGE QUOTED: 31,1s cold then 28,1s warm, solo, on a box whose background load I do not
  control. The WARM figure is the row; the spread is the honest uncertainty. It went to the SLOW half
  deliberately - the fast half measured 493-550s against a 600s foreground cap and had already taken
  gitignore-policy.Tests.ps1 the same day, so a 28s addition there is two additions to a half at 92% of
  a hard cap. The slow half is backgrounded and past its cap already, so it absorbs the cost.
  THE FIRST SUITE IN scripts/tests TO INVOKE PYTHON. Nearly the whole figure is process launch: ten rows
  x (one python start + one `git show`). It SKIPS VISIBLY where python is absent rather than failing, and
  it probes by RUNNING python, not by resolving it - `Get-Command python` returns the WindowsApps stub on
  this box, which on a machine without python launches the Microsoft Store instead.
...
check-agy-discipline-skills.Tests.ps1            43,2s  112 tests   <- FAST, re-measured 2026-09-13 (+2, section-15 panel-marker rows; was 110)
  ...
  the verdict forms and envelope steps (+1).
  107 -> 110 the same day (round 2, auditing that fold): the round-cap row went to all four disciplines,
  because planted in one it stayed green when the branch was narrowed to skip another. Every added row is
  ONE in-process linter run (~0,5s); nothing spawns a child pwsh. Runtime NOT re-measured: roughly +8s on
  the fast half, which sits near its 600s cap.
  ...
  ** THE FAST-HALF FLAG IS NOW THE LOUDEST THING ON THIS ROW. ** 6,6s at 43 tests on 2026-08-05;
  ...
  lefthook's pre-push does not run this suite; only CI does, and nothing had been pushed. The 6,6s figure
  is from 2026-08-05 at 43 tests and is NOT re-measured here - the row's COUNT is mechanically enforced
  by test-suite-registration.Tests.ps1, its DURATION is not, so treat the seconds as indicative.
...
check-capstone-new-code.Tests.ps1               107,0s   35 tests   <- SLOW, NEW 2026-09-04 (ROADMAP s24 trigger). Two runs on an idle machine, 108,2s cold / 107,0s warm; background load uncontrolled.
...
check-knowledge-store.Tests.ps1              163-252s  21 tests   <- SLOW. Each case builds a THROWAWAY
                                                                      ...
                                                                      reason.
                                                                      RANGE, NOT A POINT: 163,0s and 252,2s
                                                                      on the same commit minutes apart. The
                                                                      spread is real and UNCONTROLLED - this
                                                                      box is never idle, and two rows CLONE
                                                                      the repo. Size the slow half against
                                                                      the UPPER figure.
...
check-roadmap-claims.Tests.ps1                  54-55s   28 tests   <- SLOW, NEW 2026-08-31. Two runs,
                                                                      ...
                                                                      COUNT 8 -> 10 at AGY-CAPSTONE
                                                                      round 1: an UNREADABLE row (a
                                                                      TRACKED file absent from the
                                                                      worktree used to throw an unhandled
                                                                      FileNotFoundException and exit 1 -
...
release-lib.Tests.ps1                             5,0s   45 tests   <- FAST, re-measured 2026-08-05; 23->20 on 2026-09-14 (ghidrust full retirement removed the Get-GhidrustChannel Describe); 20->21 on 2026-09-18 (Update-Changelog must emit a pure-ASCII section, so a release cannot re-break the injected-context gate); 21->36 on 2026-09-22 (the four -Resume functions: Get-DanglingReleaseCommits, Test-ResumeState, Get-DropConflictStatus, Invoke-DropReleaseCandidate); 36->37 on 2026-09-22 (AGY-CAPSTONE round 1 fold: merge topology must survive the candidate drop); 37->42 on 2026-09-22 (AGY-TEST-AUDIT round 1 fold: the fail-CLOSED pair for an unresolvable origin/main, the --basic-regexp guard under grep.patternType=extended, the tip-candidate-is-also-root short-circuit, and the catch arm under $PSNativeCommandUseErrorActionPreference); 42->43 on 2026-09-22 (AGY-CAPSTONE round 3 fold: the audit's own fix refused a VIRGIN repo's first release, so the range now WIDENS to HEAD when origin/main does not resolve - three rows pin both halves); 43->44 on 2026-09-22 (AGY-CAPSTONE round 4 fold: that widening was DESTRUCTIVE when the tracking ref was merely pruned - it offered a SHIPPED release for dropping - so the missing-ref case now asks the REMOTE and refuses when main exists there); 44->43 on 2026-09-22 (AGY-CAPSTONE round 5: every attempt to INFER the pushed set was measured destructive or wrong, so origin/main is now REQUIRED and anything else REFUSES - three inference rows collapse into two refusal rows); 43->44 on 2026-09-22 (AGY-CAPSTONE round 6: Test-ResumeState must REPORT the refusal as a Problem, not let the callee's throw escape and discard the problems already gathered); 44->45 on 2026-09-22 (AGY-CAPSTONE round 7: that catch must be TYPED, since a bare one was measured laundering a programming error into a polite refusal). The 5,0s time is the 2026-08-05 figure and was NOT re-measured when those 15 tests landed, so it now understates this row
release.Tests.ps1                                56,5s    8 tests   <- SLOW, ADDED 2026-09-22 with release.ps1 -Resume. The FIRST suite release.ps1 has ever had. SLOW on a measured cadence judgement, not a threshold: at 56,5s it would have been the slowest member of the FAST half (whose largest was 43,2s, and that one carries 112 tests against these 6) and would have added ~75% to a ~75s inner loop. 9,4s per test is structural - each test spawns a pwsh running a COPY of release.ps1 in a throwaway repo with a real bare origin, because release.ps1 derives $RepoRoot from $PSScriptRoot and so always acts on the repo its own copy sits in. MEASURED decomposition: git setup ~3,4s/test, the irreducible pwsh+release.ps1 invocation ~3,9s/test, so amortising setup into BeforeAll would reach ~25-35s, NOT the "<5s" a reviewer predicted. The RESUME LOGIC is not gated here - it lives in release-lib.ps1 and its 45 tests stay FAST; this suite covers only the orchestrator wiring. `just test-scripts` globs the directory, so it runs in the release pre-flight regardless of this half
rule-runner.Tests.ps1                                ?   11 tests   <- FAST, ADDED 2026-09-11 (ROADMAP section 30b,
                                                                      AGY-CAPSTONE section 30 rounds 4-6). NO SOLO
                                                                      TIME - NOT MEASURED. Ten rows are pure
