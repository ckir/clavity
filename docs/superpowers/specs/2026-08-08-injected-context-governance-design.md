...
- **RESOLVE-THEN-ASSERT** - bare filenames with a known shipped extension, resolved against the domain
  roots rather than the repo root. This is the class that catches C2 (`agy-first-brainstorm.sh` resolves
  nowhere under `plugin/hooks/`).
