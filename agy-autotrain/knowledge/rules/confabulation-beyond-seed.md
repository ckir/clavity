...
- CONFABULATION, beyond SEED: it BLOCKS on an assumed-uniform code property it INFERRED rather than read,
  and never quote its structural description of a file back into a document. When seeding a known defect,
  ask for an open hunt WITHOUT naming the class, or require a proof against the actual failing operation
  per hit - named, it flags every superficially-similar construct without checking the mechanism.
