...
oldest="$(awk 'function flush(){ if(cur!=""){ if(m==""||cur<m) m=cur }; cur="" } /^## Pending/{p=1;next} /^## /{ flush(); p=0 } p && /^- \[/ { flush(); inrec=1 } p && (/^[ \t]*$/ || /^[ \t]*<!--/) { flush(); inrec=0 } p && inrec { s=$0; while(match(s,/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/)){ cur=substr(s,RSTART,10); s=substr(s,RSTART+10) } } END{ flush(); print m }' "$OBS" 2>/dev/null)"
[ -z "$count" ] && exit 0
