...
- **Task 7.1 — clavity-classic Inno installer** (the setup wrapped into `[Run]`: agentmemory MCP, **agy-mcp-bridge MCP (`delegate_to_antigravity`)**, `GEMINI.md` doorbell rule, `tmux.conf`). Slots into the same chooser. Defined in install-arch spec "Follow-on." **agy-mcp-bridge inclusion (added 2026-06-29, user request; refined by an AGY-AFTER audit):** the classic bridge is part of clavity-classic but is currently hand-registered in `~/.claude.json` (Claude side) as `uv run --directory <claudavity> --env-file <.env> python server.py`, providing `delegate_to_antigravity`. A clean shipped install (no `claudavity` working copy) must address ALL of:
  - **Bridge runtime — NOT toolchain-free.** Bundling `claudavity/server.py` under `{app}` and launching via `uv run python` STILL requires a `uv`+Python runtime on the host — unlike Task 7.8 (which ships a *compiled* binary needing nothing). Decide: (a) declare a Python/uv prerequisite for classic's bridge, or (b) compile the bridge to a standalone exe (e.g. PyInstaller) for true parity. Do NOT claim 7.8 symmetry unless (b).
  - **agy-side substrate.** `delegate_to_antigravity` silently fails without the agy-side **`claudavity-responder`** skill AND the live psmux/tmux + agentmemory signal-bus it drives. The installer must also place the responder skill into agy's skills dir (the agentmemory MCP + `tmux.conf` are already in 7.1 scope).
  - **Secrets (`.env`).** The dev registration carries `--env-file <dev .env>`. A shipped install must NOT bundle the developer `.env` (credential leak) — enumerate the bridge's actual required env (if any) and provision it safely (empty/prompt/derive), or refactor the bridge to need none.
  - **Per-user profile target.** The `~/.claude.json` write must land in the INVOKING user's profile. The variant installers are per-user (`{localappdata}` + `HKCU`), so confirm `PrivilegesRequired=lowest` (no elevation to a different profile). The classic and dotnet installers are mutually exclusive, so only one bridge/MCP set is ever present (no collision).
  - **Scope gate (strategic).** Classic-only capability — clavity-dotnet's `clavity-ls --mcp` gives `agy_ask` but NOT `delegate_to_antigravity`. Valid only while clavity-classic stays a **maintained** shipped variant (as 7.1/7.2/7.8 assume). Corollary surfaced by the 2026-06-29 classic→dotnet migration (which had to `claude mcp remove agy-mcp-bridge`): migrating a machine classic→dotnet **regresses** autonomous code-delegation.
...
- **Task 7.7 — `clavity --restart-agy` (agy-only restart) for clavity-classic.** Add a `--restart-agy` flag (or `restart-agy` subcommand) that tears down + relaunches ONLY the agy psmux session under the same `--session` name, WITHOUT co-launching a new Claude Code. Today the only relaunch is `clavity start`, which co-launches Claude **and** agy — so there is no way to recover a hung/broken agy (e.g. its MCP servers failing to come up) while KEEPING the current driving Claude/conversation: the operator must `clavity stop` + manually relaunch, and `clavity start` would orphan the session. `--restart-agy` should re-run agy's exact launch (its env + MCP config) in the existing session and confirm readiness via a `ping` round-trip. **Surfaced 2026-06-29** when an agy MCP-server hang forced a full `clavity stop` + `tmux kill-server` teardown mid-capstone, and the missing agy-only restart orphaned the driving session.
- **Task 7.8 — Prebuild clavity-classic Rust binaries on CI (no user-side Rust required).** Add a release-CI job that `cargo build --release` the Rust `clavity` binary on a Windows runner (matrix per the porting guide: win-x64 first, then Linux/macOS) and publishes it as a Release asset, so the classic installer (Task 7.1) ships a **prebuilt `clavity.exe`** and users do NOT need the Rust/cargo toolchain. Today classic requires `cargo install --git https://github.com/ckir/clavity --branch clavity-classic` (Rust on the user's machine); this removes that requirement and is the build prerequisite for the classic `clavity-classic-setup.exe` (Task 7.2). **(User request 2026-06-29.)**
