...
- feat(assertion-strength): mirror the hook to clavity-classic
...
- feat(hooks): mirror the direct-driver capture reminder to classic
...
- fix(capstone-r11): pin the negation contract at ALL FOUR call sites - two were genuinely unpinned
- fix(capstone-r9): the CI step I added yesterday named three files it never ran
...
- fix(classic): fold capstone round 6 - all four defects were in the code I added last
- fix(classic): self-skip the two mutating integration tests, and correct a claim I made about them
...
- fix(test): close two fail-open guards an audit found in tests I wrote today
- fix(capstone-r4): the ledger claimed a check one of the two checkers cannot run
- fix(classic): the sibling A1's fold missed - allow(dead_code) on a test-only fn
- fix(classic): a test-only const declared a visibility that cannot exist
- fix(classic): scope RETIRED_LEGACY_FILE_NAME to test builds - the fourth CI red
...
- fix(capstone-r25): the deletion audit, and a trap this repo had already documented
...
- fix(injected-context): close the domain-coverage hole and PIN it with a test
...
- fix(hooks): mirror the consult-guard attribution caveat to classic
...
- fix(hooks): honour .no-agy from a subdirectory in the one hook that deletes files
