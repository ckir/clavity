...
- [anti-pattern] (driver/deterministic) The ask-transport returns the peer's FINAL message only, so a peer that delivers its findings and then appends any closing chatter ("let me know when you are ready", "awaiting your next round") DESTROYS the findings in transit - the driver receives the pleasantry. Measured: a full seated review of eleven items arrived as the single sentence "I am awaiting your next round or instructions"; the findings were still visible in the conversation trajectory and were recovered by inspecting it. Two mitigations, use both: instruct in the payload that the findings MUST be the final message with nothing appended after, and when a reply looks anomalously short for the work requested, read the trajectory before re-asking.  ·  `[corpus]` · 2026-08-16
