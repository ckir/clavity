...
| `check-ci-filter-coverage.ps1` | Gate: `ci-scripts.yml`'s `paths:` filter must cover what the scripts suite asserts against. Check A pins a required-entry table (each with its reason) and asserts every entry appears in BOTH the `push` and `pull_request` blocks; Check B fails closed on any git-tracked top-level tree a suite names but no entry reaches, via the PowerShell AST so `[IO.Path]::Combine` segments are seen too | `pwsh -File scripts/check-ci-filter-coverage.ps1` |
