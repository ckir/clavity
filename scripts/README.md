...
| `check-skill-frontmatter.ps1` | Fail if any tracked `SKILL.md` lacks `name`/`description` frontmatter, has a `name` that differs from its directory, a multi-line `description`, or a `description` over 440 UTF-8 bytes - Claude Code silently stops ADVERTISING a skill whose description is too long. Zero targets or a stale exclusion is exit 2, never a pass | `just check-skill-frontmatter` (lefthook pre-push; CI `ci-scripts.yml`) |
