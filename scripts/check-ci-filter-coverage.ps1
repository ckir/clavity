...
$Required = [ordered]@{
    ...
    'agy-autotrain/hooks/**'                            = 'FIVE suites assert against this tree - agy-curate-nudge, agy-inbox-snapshot, agy-learn-reminder, plugin-hooks-payload and plugin-hooks-registration - and ci-scripts.yml runs them by DIRECTORY SWEEP (Invoke-Pester scripts/tests), so they are invisible to a by-name search of this workflow. Until 2026-08-27 no workflow in the repo triggered on the tree at all: a commit touching only these four tracked files merged with every suite guarding them unrun. Listed as a TREE and NOT narrowly, unlike the cheatsheet row: the narrow-listing rationale in ci-scripts.yml rejects the WHOLE agy-autotrain tree because it would fire on every knowledge-inbox capture, and this subdirectory holds no inbox'
